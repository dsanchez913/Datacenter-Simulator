...
A=con_total(Nweb_opt,Napp_opt,Ndb_opt,double(Non));
 solution = [double(Objective),Nweb_opt,Napp_opt,Ndb_opt,round(double(Non)),round(double(Noff)),min(Nwk,Ns),double(xgrid),double(Gnr),double(Gnrs), double(Grs),double(Tav),costo_3,costo_4,A];
else
