...
costo_4=double(Gnrs)*Pnr;
 solution = [double(Objective),Nweb_opt,Napp_opt,Ndb_opt,round(double(Non)),round(double(Noff)),round(double(Nweb))+round(double(Napp))+round(double(Ndb)),double(xgrid),double(Gnr),double(Gnrs), double(Grs),double(Tav)];
else
